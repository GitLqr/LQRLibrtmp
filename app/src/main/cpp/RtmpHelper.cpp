...
extern "C"
JNIEXPORT void JNICALL Java_com_lqr_librtmp_RtmpHelper_n_1pushSPSPPS
  (JNIEnv *env, jobject instance, jbyteArray sps_ , jint spsLen, jbyteArray pps_, jint ppsLen){
    jbyte  *sps = env->GetByteArrayElements(sps_,NULL);
    jbyte *pps = env->GetByteArrayElements(pps_,NULL);
    if(rtmpPush && !isExit){
        rtmpPush->pushSPSPPS(reinterpret_cast<char*>(sps),spsLen, reinterpret_cast<char*>(pps),ppsLen);
    }
...
extern "C"
JNIEXPORT void JNICALL Java_com_lqr_librtmp_RtmpHelper_n_1pushVideoData
  (JNIEnv *env, jobject instance, jbyteArray data_, jint dataLen, jboolean keyFrame){
...
extern "C"
JNIEXPORT void JNICALL Java_com_lqr_librtmp_RtmpHelper_n_1pushAudioData
  (JNIEnv *env, jobject instance, jbyteArray data_, jint dataLen){
    jbyte *data = env->GetByteArrayElements(data_,NULL);
    if(rtmpPush && !isExit){
...
extern "C"
JNIEXPORT void JNICALL Java_com_lqr_librtmp_RtmpHelper_n_1stop
